...
public class FrequencyActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button save_btn = findViewById(R.id.button_save_speed); // Make sure the correct button ID matches your XML
        ...
        save_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String fromUnit = autoCompleteTextView_from.getText().toString();
                String toUnit = autoCompleteTextView_to.getText().toString();
                String input = outlinedTextField_frequency_unit.getText().toString();
                String result = frequency_output_text.getText().toString();

                if (fromUnit.isEmpty() || toUnit.isEmpty() || input.isEmpty() || result.isEmpty()) {
                    Toast.makeText(FrequencyActivity.this, "Make sure you've done a conversion first!", Toast.LENGTH_SHORT).show();
                    return;
                }
    ...
    private void saveConversionToDatabase(String fromUnit, String toUnit, String input, String result) {
        DBHelper dbHelper = new DBHelper(FrequencyActivity.this);
        boolean isInserted = dbHelper.insertFrequencyConversion(fromUnit, toUnit, input, result);

        if (isInserted) {
            Toast.makeText(FrequencyActivity.this, "Conversion saved!", Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(FrequencyActivity.this, "Error saving conversion.", Toast.LENGTH_SHORT).show();
        }
    }
